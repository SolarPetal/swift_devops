...
// rollbackPipelineRun 回滚到部署历史中选中的成功记录。
// 后端只接受 status=success 且关联了 Bundle / Artifact 的历史。
export async function rollbackPipelineRun(id: number): Promise<PipelineRun> {
  const r = await api.post<PipelineRun>(`/pipelines/${id}/rollback`)
  return r.data
}
