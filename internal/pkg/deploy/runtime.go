...
// Runtime 一台主机上一个服务的「部署 + 生命周期」原语抽象（Sprint X.10）。
//
// 设计意图：把"写启动文件 + 重启 + 等 active + 诊断 + 停止"从 deployHost 里抽出来，
// 让 systemd / nohup（以及未来 docker / k8s）各自实现，deployHost 只管编排阶段。
//
// 命名约定：本接口的"Unit"沿用 systemd 术语，对 nohup 模式而言指 start.sh + stop.sh 这套启动脚本。
type Runtime interface {
	// Name 模式标识："systemd" / "nohup"，用于日志和错误归因。
	Name() string

	// PrepareUnit 在远端写下启动需要的文件（systemd 写 unit，nohup 写 start.sh / stop.sh 与 pid 目录）。
	...
	//   - systemdRuntime 调用时 → 干掉同 spec 的 nohup pid / 进程
	//   - nohupRuntime 调用时 → systemctl stop + disable 同名 unit
...
func ValidateDeployMode(mode string) error {
	m := strings.ToLower(strings.TrimSpace(mode))
	switch m {
	case "", DeployModeSystemd, DeployModeNohup:
		return nil
	}
	return fmt.Errorf("deploy_mode 仅支持 systemd / nohup（实际：%q）", mode)
}
